function [Aeq, beq] = getAbeq(n_seg, n_order, ts, start_cond, end_cond)
    ...
    Aeq_start(1,1) = 1; % c0
    Aeq_start(2,1:2) = n_order*[-1,1]; % c'0 = n*(-c0 + c1)
    Aeq_start(3,1:3) = n_order*(n_order-1)*[1,-2,1]; %  c''0 = n*(n-1)*(c2 -2*c1 +c0)    
    Aeq_start(4,1:4) = n_order * (n_order-1) * (n_order-2) * [1, -3, 3, -1]; % c'''0 = n*(n-1)* (n-2) *(c3 - 3*c2 + 3*c1 - c0)   
    ...
    Aeq_end(1,end) = 1 ;% cn
    Aeq_end(2,end-1:end) = n_order * [-1, 1];% c'n-1 = n*(cn -cn-1)
    Aeq_end(3,end-2:end) = n_order * (n_order-1) * [1, -2, 1];% c''n-2 = n^2*(n-1)*(cn - 2*cn-1 + cn-2)    
    Aeq_end(4,end-3:end) = n_order * (n_order-1) * (n_order-2) * [1, -3, 3, -1];% c''n-3 = n*(n-1)*(n-2)*(cn - 3*cn-1 + 3*cn-2 - c0)   
    ...
    for k = 1:n_seg-1 % (c(n))- 3*c(n-1) + 3*c(n-2) - c(n-3)) segment 1 + (-c3 + 3*c2 - 3*c1 + c0) segment 2
        Aeq_con_j(k,k*(n_order+1)-3:k*(n_order+1)) = [1, -3, 3, -1] ;
        Aeq_con_j(k,k*(n_order+1)+1:k*(n_order+1)+4) = [-1, 3, -3, 1];
    end  
