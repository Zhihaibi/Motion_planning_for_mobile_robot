...
Vector3d getPosPoly( MatrixXd polyCoeff, int k, double t )
{
    ...
    for ( int dim = 0; dim < 3; dim++ )
    {
        ...
        ret(dim) = coeff.dot(time); // coeff is from low to high
//        cout << "dim:" << dim << endl;
...
VectorXd timeAllocation(MatrixXd Path)
{
    ...
    */
    ...
    double _Acc = 1.0;
    double s = 0;
    double t_slope = _Acc/_Vel;
    double max_s = 0.5*_Acc*t_slope*t_slope;
    for(int i = 0; i<Path.rows()-1; i++)
    {
        a = Path.block<1,3>(i,0);
        b = Path.block<1,3>(i+1,0);
        c = a-b;
        s = c.norm();
        if(s <= max_s)
        {
            time(i) = sqrt(2*s/_Acc);
        }
        else
        {
            time(i) =  (s - max_s)/_Vel + t_slope;
        }
    }
    return time;
}