...
{
    double optimal_cost ; // this just to initial the optimal_cost, you can delete it
    /*
    ...
    */
    double T = 1.25;
    Eigen::Vector3d _target_velocity = {0,0,0};
    Eigen::Vector3d delta_p = _target_position - _start_position;
    Eigen::Vector3d delta_v = _target_velocity - _start_velocity;
    double alpha1 = -12/pow(T,3) * delta_p(0) + 6/pow(T,2) * delta_v(0);
    double alpha2 = -12/pow(T,3) * delta_p(1) + 6/pow(T,2) * delta_v(1);
    double alpha3 = -12/pow(T,3) * delta_p(2) + 6/pow(T,2) * delta_v(2);
    double beta1 = 6/pow(T,2) * delta_p(0) - 2/T * delta_v(0);
    double beta2 = 6/pow(T,2) * delta_p(1) - 2/T * delta_v(1);
    double beta3 = 6/pow(T,2) * delta_p(2) - 2/T * delta_v(2);
    optimal_cost = T + 1/3*pow(alpha1,2)*pow(T,3) + alpha1*beta1*pow(T,2) + pow(alpha1,2)*T
                     + 1/3*pow(alpha2,2)*pow(T,3) + alpha2*beta2*pow(T,2) + pow(alpha2,2)*T
                     + 1/3*pow(alpha3,2)*pow(T,3) + alpha3*beta3*pow(T,2) + pow(alpha3,2)*T;
    return optimal_cost;
}
